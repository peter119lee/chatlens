...
const SUMMARY_RULES = [
  ...
  "actions 只列明确请求某人执行、某人明确承诺执行、或带明确责任/截止时间的后续事项。普通提问、求购、求推荐、征询意见、情绪表达和已经得到一般回复的问题不得列为 action，可放入 topic 或 uncategorized。必须检查后续消息：如果明确事项已经完成，status 用 resolved 并在 resolution 写一句是谁怎么解决的；仍需执行的才用 open。",
  ...
  user: JSON.stringify(
    {
      ...
      rules: [
        ...
        "actions 最多 15 条，只保留明确请求某人执行、明确承诺执行、或带责任/截止时间的事项；普通提问、求购、求推荐、征询意见不得变成 action。优先未解决(open)的；跨段判断状态：某段还是 open、后段明确完成的，改成 resolved 并在 resolution 说明是谁怎么解决的；仍需执行才保留 open。相同事项去重。",
