...
**核心原则：只读、离线。** 工具只复制 QQNT 的本地数据库文件做离线解析 —— 不使用 QQ 登录协议、不发送消息、不修改任何 QQ 文件。唯一的例外是「自动获取密钥」：它会**只读地**读取本机 QQ 进程内存以取出数据库密钥（不注入代码、不修改 QQ），是一次性本地操作。
...
- **腾讯能发现我在用这个工具吗？** 工具不登录、不联网访问 QQ 服务器、不使用登录协议，只读取数据库文件的副本，因此不存在协议层面的检测面。「自动获取密钥」只在本机只读地读取 QQ 进程内存，属于纯本地行为、不产生任何网络流量。唯一涉及腾讯服务器的流量是浏览器加载公开头像 CDN（与正常看网页加载头像相同）。这与 NapCat / OneBot 这类需要登录协议的机器人框架有本质区别。
...
- 本地消息库默认 30 天自动清理（设置页可调）；运行 `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\cleanup_generated_data.ps1` 可一键清空全部生成数据。
